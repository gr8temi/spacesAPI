...
class CreateSpace(APIView):
    ...
    def post(self, request, format="json"):
        ...
        existing = Space.objects.filter(name=name, space_type=space_type)
        ...
        space_host = Agent.objects.get(agent_id=uuid.UUID(data.get("agent")))
        no_of_spacehost_spaces = Space.objects.filter(agent=space_host).count()
        if no_of_spacehost_spaces > 0 and space_host.document_verified is False:
            return Response(
                {"message": "Kindly upload a document to verify yourself before proceeding to create the space"},
                status=status.HTTP_400_BAD_REQUEST,
            )
        with transaction.atomic():
            try:
                user_id = Agent.objects.get(
                    agent_id=uuid.UUID(data.get("agent"))
                ).user.user_id  # TODO: Catch error if it fails
                ...
                if bool(existing):
                    return Response(
                        {
                            "message": f"Space with name {name} already exists in this category"
                        },
                        status=status.HTTP_400_BAD_REQUEST,
                    )
                    ...
                    subscriber.send(
                        ...
                        data={
                            "name": space_id,
                            "user_id": f"{user_id}",
                            "notification": f"{space_name} was successfully created",
                        },
                    )

                    return Response(
                        {
                            "payload": spaceDataSerializer.data,
                            "message": f"{name} was created successfully",
                        },
        ...
        return Response(
            {
                "message": "Check your input, some fields might be missing",
                "error": spaceDataSerializer.custom_full_errors,
            },
            status=status.HTTP_400_BAD_REQUEST,
        )
